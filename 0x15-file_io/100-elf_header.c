...
/**
 * check_elf - a function that checks ELF header
 ...
 * @elf: file descriptor of the ELF.
 *
 * Description: If the file cannot be closed - exit98.
 */
void close_elf(int elf)
{
	if (close(elf) == -1)
...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	r = read(fp, header, sizeof(Elf64_Ehdr));
	if (r == -1)
	{
		free(header);
		close_elf(fp);
